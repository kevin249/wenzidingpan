...
=======
- **两种行内样式**：左中右（左右各两行文字）/ 上中下（上下各一行文字），中间永远是 K 线
- **K 线高度可调**：留 0 按字号自动推算，也可以直接指定像素高度，拖拽窗口时等比缩放，窗口放不下时自动压扁
- **字体可改**：字体与字号都能设置，界面所有字号按基准字号等比缩放
- **拖拽缩放**：拖右下角只改变窗口外框，内部行情根据可用宽高自动缩放和重排，不使用滚动条
>>>>>>> 5127ecc26ae3ee3f3c09ffe26bd6167874143b6b
- **背景可调**：背景颜色和不透明度独立于窗口透明度，可以调到完全透明只留文字
- **鼠标穿透**：开启后点击直接落到下层程序，只在左上角留一个可拖动的小把手
- **按钮可隐藏**：关掉右上角的刷新 / 设置 / 灰度 / 退出后整条标题栏收起，只剩行情内容，托盘菜单里能开回来
...
=======
- **东方财富暗盘资金**：每只股票显示当日暗盘资金，统一以亿为单位保留两位小数，红正绿负（跟随涨跌配色）
...
| 右上角按钮 | 开 | 刷新 / 设置 / 灰度 / 退出；关掉后整条标题栏收起，托盘里也能开关 |
| 暗盘资金 | 开 | 显示东方财富当日暗盘资金 |
...
- 关掉右上角按钮后，刷新与退出走托盘菜单；托盘也不显示时，用启动时打印的设置页地址把按钮开回来
