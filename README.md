...
- **按行数平铺**：设几行就铺几行，股票依次向右排开——设 1 行就是一条横向行情带，窗口尺寸自适应
- **两种显示模式**：网格平铺 / 单行横向滚动（鼠标悬停暂停）
- **字体可改**：字体与字号都能设置，界面所有字号按基准字号等比缩放
- **拖拽缩放**：拖右下角把手改变窗口大小，字体与走势图按比例一起放大
- **背景可调**：背景颜色和不透明度独立于窗口透明度，可以调到完全透明只留文字
- **鼠标穿透**：开启后点击直接落到下层程序，只在左上角留一个可拖动的小把手
- **数据源自动选择**：默认按东财 → 腾讯 → 新浪自动挑一个能用的并记住它，也可手动指定
- **自选股搜索**：输入代码、名称、拼音或首字母（如 `gzmt`）即可搜到并加进列表，可排序删除
...
网格行数设为 1，所有自选横向铺成一条行情带：

![一行平铺](docs/screenshot-tile-one-row.png)

背景不透明度调到 0：只剩文字和曲线浮在桌面上。

![透明背景](docs/screenshot-transparent.png)
...
| --- | --- | --- |
| 数据源 | 自动 | 见下表 |
| 自选股票 | `600519 000001 300750 601318` | 搜索框支持代码/名称/拼音/首字母，最多 50 只 |
| 显示模式 | 网格平铺 | 可切换为单行横向滚动 |
| 网格行数 | 4 | 1–30 行；填 1 全部横向排开，填 2 铺两行，窗口宽高随之自适应；单行模式下不生效 |
| 字体 | 跟随系统 | 填系统里已安装的字体名，如 `Microsoft YaHei` |
| 字号 | 13px | 9–28px，界面整体等比缩放 |
| 刷新间隔 | 5 秒 | 1–3600 秒 |
| 涨跌配色 | 红涨绿跌 | 可切换为绿涨红跌 |
| 窗口透明度 | 95% | 20%–100%，整窗一起变淡，文字也会淡 |
| 背景颜色 | `#11141c` | 只影响背景板 |
| 背景不透明度 | 82% | 0%–100%，调到 0 就只剩文字和曲线浮在桌面上 |
| 鼠标穿透 | 关 | 开启后窗口不拦鼠标，只有左上角把手能拖；托盘里也能开关 |
| 最前显示 | 开 | 关掉后组件会被其他窗口盖住 |
| 暗盘资金 | 开 | 显示东方财富当日暗盘资金 |
| 走势图 | 开 | 紧凑模式下会压扁，但不会隐藏 |
| 走势图用分时数据 | 开 | 关掉后不联网取分时，只画组件运行期间的采样点 |
| 紧凑模式 | 关 | 隐藏暗盘与页脚，走势图保留 |
...
| 数据源 | 说明 | 覆盖范围 |
| --- | --- | --- |
| `auto` | **默认**，按下面的顺序逐个试，用第一个真出数的并记住它 | 沪 / 深 / 北 |
| `eastmoney` | 东方财富 push2 接口 | 沪 / 深 / 北 |
| `tencent` | 腾讯行情 `qt.gtimg.cn` | 沪 / 深 / 北 |
| `sina` | 新浪财经 `hq.sinajs.cn` | 沪 / 深 / 北 |
| `mock` | 离线随机游走，不联网、不需要密钥 | 任意代码 |

`auto` 会按东财 → 腾讯 → 新浪的顺序逐个试，用第一个真出数的，并记住它下次优先用；
全都不出数时保留第一个源的错误原因。状态栏会显示当前实际走的是哪家。
...
stockwidget/
  ...
  search.py          股票搜索（东财 suggest，支持拼音与首字母）
  providers/
  ...
  ui/
    ...
    quote_row.py     网格里的一格行情（名称/暗盘 · 分时图 · 现价/涨跌）
    marquee.py       单行滚动跑马灯
...
暗盘显示、分时曲线与基准线、按行数平铺成网格、紧凑模式仍保留走势图、拖拽缩放后字号等比放大、背景全透明、
鼠标穿透与左上角把手、单行模式，以及**从 WebUI 改配置后桌面窗口是否跟着变**。
